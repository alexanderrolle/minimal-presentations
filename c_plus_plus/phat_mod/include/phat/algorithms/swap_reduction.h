...
namespace phat {
    ...
    public:
        ...
        void operator() ( boundary_matrix< Representation >& boundary_matrix ) {
		  ...
		  //std::cout << "Compare: " << boundary_matrix.size(cur_col) << " and " << boundary_matrix.size(lowest_one_lookup[ lowest_one ]) << std::endl;
		  //std::cout << "Again  : " << boundary_matrix.get_num_rows(cur_col) << " and " << boundary_matrix.get_num_rows(lowest_one_lookup[ lowest_one ]) << std::endl;
		  if(boundary_matrix.size(cur_col) < boundary_matrix.size(lowest_one_lookup[ lowest_one ])) {
		    //std::cout << "SWAPPING " << lowest_one_lookup[ lowest_one ] << " and " << cur_col << std::endl;
